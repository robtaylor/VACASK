    ...
    def collect_masters(self, lines, depth=0):
        ...
        for ll in lines:
            ...
            if isinstance(eol, list):
                self.collect_masters(eol, depth+1)
                continue
            ...
            if l.startswith(".model"):
                ...
                if mtype in self.cfg["type_map"]:
                    ...
                    builtin = True
                    extra_params, family, has_level, has_version = self.cfg["type_map"][mtype]
                    
                    # Collect level and version if needed and remove them
                    pnew = []
                    for pname, pval in params:
                        if pname=="level" and has_level:
                            level = int(pval)
                            continue
                        elif pname=="version" and has_version:
                            version = int(pval)
                            continue
                        else:
